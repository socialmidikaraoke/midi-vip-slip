...
TARGET_ACCOUNT_NUMBER = '020300995519' # เลขบัญชีที่ต้องตรวจสอบ
...
# ================= ฟังก์ชันช่วย (AI อ่านสลิป) =================
def analyze_slip_image(image_bytes):
    """
    คืนค่า: (time_obj, is_correct_account)
    time_obj: เวลาที่อ่านได้ (หรือ None)
    is_correct_account: True/False ว่าเจอเลขบัญชีปลายทางไหม
    """
    try:
        ...
        # 2. หาเลขบัญชี (Account Check)
        # ลบตัวอักษรทุกอย่างที่ไม่ใช่ตัวเลขออก เพื่อให้หาเจอง่ายขึ้น (เช่น 020-300... จะกลายเป็น 020300...)
        digits_only = re.sub(r'\D', '', full_text)
        is_correct_account = TARGET_ACCOUNT_NUMBER in digits_only
        
        return found_time, is_correct_account
...
st.info(f"กรุณาโอนเงินเข้าบัญชี: **ออมสิน {TARGET_ACCOUNT_NUMBER}** เท่านั้น")

with st.form("slip_form"):
    uploaded_file = st.file_uploader("1. อัปโหลดรูปสลิป (ต้องชัดเห็นเวลาและเลขบัญชี)", type=['png', 'jpg', 'jpeg'])
    
    detected_time = None
    account_check_passed = False
    
    if uploaded_file:
        with st.spinner("⏳ กำลังตรวจสอบเอกสาร..."):
            # เรียกใช้ฟังก์ชันใหม่ที่คืนค่า 2 ตัว
            t, acc_ok = analyze_slip_image(uploaded_file)
            uploaded_file.seek(0)
            
            if t:
                detected_time = t
                account_check_passed = acc_ok
            else:
    ...
    if submit_btn:
        if not uploaded_file: 
            ...
            st.error("❌ ไม่สามารถทำรายการได้ เนื่องจากสลิปไม่ชัดเจน (อ่านเวลาไม่ได้)")
        elif not account_check_passed:
            # แจ้งเตือนกรณีเลขบัญชีไม่ตรง (หรืออ่านไม่ออก)
            st.error(f"❌ ไม่สามารถทำรายการได้: ระบบไม่พบเลขบัญชีปลายทาง {TARGET_ACCOUNT_NUMBER} ในสลิป หรือสลิปไม่ชัดเจน")
            st.warning("กรุณาถ่ายภาพให้เห็นเลขบัญชีปลายทางชัดเจน หรือติดต่อแอดมิน")
        elif not sender_name: 
            st.error("❌ ลืมใส่ชื่อ")
        elif amount % 100 != 0: 
            st.error("❌ ยอดเงินต้องเต็มร้อย")
        else:
            try:
                ...
                if client:
                    ...
                    # 1. เช็กประวัติซ้ำ (Time Window Protection)
                    ...
                    if is_duplicate:
                        ...
                        st.warning(f"กรุณาตรวจสอบว่าท่านได้ทำรายการไปแล้วหรือไม่")
                    else:
                        # 2. ค้นหาและอัปเดต
                        ...
                        if found_row_index:
                            ...
                            log_ws.append_row([str(datetime.now()), sender_name, amount, str(detected_time), "Success (Verified Acc)"])
